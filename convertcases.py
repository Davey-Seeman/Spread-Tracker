...
for line in datarows:
	...
	caseradius = int(confirmed) * 5
	
	if (int(confirmed) > 0):
		if (prov != ""):
			marker = "L.circle([" + lat + "," + lon + "],{color:'red',fillColor:'#f03',fillOpacity:0.5,radius:" + str(caseradius) + "}).addTo(layergroup).bindPopup('" + prov.replace("'", "") + "," + country.replace("'","") + " : " + confirmed + "')"	
		else:
			marker = "L.circle([" + lat + "," + lon + "],{color:'red',fillColor:'#f03',fillOpacity:0.5,radius:" + str(caseradius) + "}).addTo(layergroup).bindPopup('" + country.replace("'", "") + " : " + confirmed + "')"
