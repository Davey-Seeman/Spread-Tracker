...
for line in datarows:
	...
	deathsradius = int(deaths) * 25
	
	if (int(deaths) > 0):
		if (prov != ""):
			marker = "L.circle([" + lat + "," + lon + "],{color:'red',fillColor:'#f03',fillOpacity:0.5,radius:" + str(deathsradius) + "}).addTo(layergroup).bindPopup('" + prov.replace("'", "") + "," + country.replace("'","") + " : " + recover + "')"	
		else:
			marker = "L.circle([" + lat + "," + lon + "],{color:'red',fillColor:'#f03',fillOpacity:0.5,radius:" + str(deathsradius) + "}).addTo(layergroup).bindPopup('" + country.replace("'", "") + " : " + deaths + "')"
